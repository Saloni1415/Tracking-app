...
@Component({
  selector: 'app-catalogg',
  templateUrl: './catalogg.component.html',
  styleUrls: ['./catalogg.component.scss']
})
export class CataloggComponent implements OnInit {
  ...
  public openProductDialog(product){   
    ...
    dialogRef.afterClosed().subscribe(product => {
      if(product){
        this.router.navigate(['/catalogg', product.id, product.name]); 
      }
    });
  }
  ...
  public onChangeCategory(event){
    if(event.target){
      this.router.navigate(['/catalogg', event.target.innerText.toLowerCase()]); 
    }   
  }
